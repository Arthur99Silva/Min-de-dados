...
GRAFICO_RANKING_FILENAME = "ranking_paises_previsto_xgboost_gscv.png" 
TOP_N_GRAFICO = 20
...
if current_top_n > 0:
    ...
    barplot = sns.barplot(
        x='Pontuação de Felicidade Prevista (XGBoost_GSCV)', 
        y='País',
        data=df_plot,
        palette='crest_r' 
    )
    plt.title(f'Top {current_top_n} Países (XGBoost_GSCV) por Pontuação Prevista', fontsize=16)
    plt.xlabel('Pontuação de Felicidade Prevista (XGBoost_GSCV)', fontsize=14) 
    plt.ylabel('País', fontsize=14)
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=10)
    for index, value in enumerate(df_plot['Pontuação de Felicidade Prevista (XGBoost_GSCV)']): 
